...
def answer_one():
    ...
    for degree,counter in zip([1,3,6,9],[0,1,2,3]):
...
def answer_two():
    ...
    for degree in range(0,10):
        ...
        r2_train[degree] = r2_score(y_train, y_train_predict)
        r2_test[degree] = r2_score(y_test, y_test_predict)
...
def answer_three():
    ...
    #plt.figure(figsize=(10,5))
    #plt.plot(degree, r2_train, lw=2, label='R2 training data')
    #plt.plot(degree, r2_test, lw=2, label='R2 training data')
    #plt.legend(loc=0)
...
def answer_four():
    ...
    #Predictions
    y_test_predict_lr = lr.predict(X_test_poly)
    y_test_predict_lasso = lasso.predict(X_test_poly)
        
    #R2
    r2_lr = r2_score(y_test, y_test_predict_lr)
    r2_lasso = r2_score(y_test, y_test_predict_lasso)
        

    return r2_lr, r2_lasso
...
def answer_five():
    ...
    feature_importances = pd.DataFrame({'feature': X_train2.columns,
                                        'importance': clf.feature_importances_})
    
    return list(feature_importances.sort(['importance'],ascending = False).feature[0:5])
...
def answer_seven():
    ...
    #score_train = answer_six()[0]
    #score_test = answer_six()[1]
    #gamma = np.logspace(-4,1,6)
    
    #plt.figure(figsize=(10,5))
    
    #plt.semilogx(gamma, score_train, lw=2, label='Train accuracy')
    #plt.semilogx(gamma, score_test, lw=2, label='Test accuracy')
